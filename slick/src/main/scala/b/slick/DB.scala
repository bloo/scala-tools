...
object DB extends Logger {
	...
	private def config = {
		if (configured.compareAndSet(false, true)) {
			...
			else if (cfg hasPath "uris") cfg.getConfigList("uri").toList.foreach { configEach(URIDataSource, _) }
			if (cfg hasPath "pool") configEach(PooledDataSource, cfg getConfig "pool")
			else if (cfg hasPath "pools") cfg.getConfigList("pool").toList.foreach { configEach(PooledDataSource, _) }
		}
	}
