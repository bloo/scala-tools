...
case class EmailAdr(email: String, name: Option[String] = None)
case class MessageContext(template: String, subject: String, from: EmailAdr, replyTo: Option[EmailAdr] = None)
...
trait Emailer extends ScalateEmailer {
	...
	class EmailMessageBuilder(ctx: MessageContext) {
	    ...
	    def send(to: EmailAdr, attributes: (String, Any)*) = {
    ...
    def compose(ctx: MessageContext): EmailMessageBuilder =
        new EmailMessageBuilder(ctx)
}
